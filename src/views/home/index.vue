<!--
 * @Author      : Mr.bin
 * @Date        : 2023-06-19 21:21:44
 * @LastEditTime: 2023-06-19 21:42:16
 * @Description : home
-->
<template>
    ...
    setOrderID(order_id) {
      ...
      this.$axios
        ...
        .then(res => {
          ...
          if (data.status === 1) {
            // 判断一下蓝牙连接与否
            if (this.$store.state.isBluetooth === false) {
              this.$alert(
                '检测到您未进行“蓝牙连接”，请点击“前往连接”按钮！',
                `提示`,
                {
                  type: 'warning',
                  showClose: false,
                  center: true,
                  confirmButtonText: '前往连接',
                  callback: () => {
                    this.$router.push({
