...
export default {
  ...
  data() {
    return {
      ...
      audioSrc: path.join(__static, `narrate/mandarin/Train/圆环保持训练.mp3`),
      ...
      smallCircle: this.$store.state.settings[0].smallCircle, // 小圆半径
      bigCircle: this.$store.state.settings[0].bigCircle, // 大圆半径
  ...
  methods: {
    ...
    handleRefresh() {
      this.$router.push({
        path: '/refresh',
        query: {
          routerName: JSON.stringify('/ring-hold-measure'),
