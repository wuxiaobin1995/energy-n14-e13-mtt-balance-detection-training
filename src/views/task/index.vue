<!--
 * @Author      : Mr.bin
 * @Date        : 2023-06-19 21:25:34
 * @LastEditTime: 2023-06-19 21:28:31
 * @Description : 任务详情页
-->
<template>
  <div class="task">
          ...
          >
            <el-carousel-item v-for="(item, index) in settings" :key="index">
              <div class="box">
                ...
                <div class="item">欢迎使用平衡测试与康复训练系统！</div>
              </div>
            </el-carousel-item>
          </el-carousel>
        </el-card>
    ...
    handleStart() {
      ...
      if (this.orderIdType === '评估') {
        for (let i = 0; i < settings.length; i++) {
          ...
          switch (item.pattern) {
            case '平衡测试':
              settingsRouter.push('balance-measure')
              break
            case '本体感觉平衡测试':
              settingsRouter.push('proprioception-balance-measure')
              break
            case '左右平衡测试':
              settingsRouter.push('lr-balance-measure')
              break
            case '前后平衡测试':
              settingsRouter.push('fb-balance-measure')
              break
            case '对角线平衡测试':
              settingsRouter.push('diagonal-balance-measure')
              break
            default:
              break
          }
        ...
        for (let i = 0; i < settings.length; i++) {
          ...
          switch (item.pattern) {
            case '圆环保持训练':
              settingsRouter.push('ring-hold-measure')
              break
            case '圆圈保持训练':
              settingsRouter.push('circle-hold-measure')
              break
            default:
              break
          }
